...
| `docs/odi-connector-readiness-analysis.md` | Historical ODI connector readiness audit: repository baseline, ingestion architecture, reusable assets, skill/harness findings, Codex/Serena setup, and researched SDK/MCP/tooling options | Reviewing the evidence that preceded the selected design; use the current handoff and explorer architecture for implementation decisions | 2026-08-29 | SUPERSEDED for product decisions by `docs/codex-work/odi-14c-context.md` and `tools/odi-lineage-explorer/docs/architecture.md` |
| `docs/oracle-odi-lab-installation.md` | Reproducible Oracle Linux 8 lab runbook with independent Database 19c, ODI 14.1.2, and VM-operations sections; includes verified media hashes, paths, RCU/Studio setup, SDK inventory, SELinux-safe startup, NAT forwarding, and secret boundaries | Installing the database alone, ODI against an existing database, or rebuilding and preserving the combined ODI connector lab | 2026-09-03 | CURRENT — database, ODI Enterprise, RCU, Studio, SQL Developer, and initial Smart Import verified; standalone agent pending |
| `docs/odi-lineage-explorer-macos-intel.md` | End-to-end Intel macOS migration runbook: powered-off OVA export/import, checksum, NAT recreation, new SSH key, direct JAR retrieval from the VM, exact Git checkpoint, toolchain, and startup | Moving the current Windows/WSL development state and Oracle lab to an Intel Mac without committing licensed binaries or secrets | 2026-09-03 | CURRENT — execute after the named checkpoint branch and tag are pushed |
| `tools/odi-lineage-explorer/docs/architecture.md` | Current product boundary, Context resolution, Load Plan→Scenario→Mapping traversal, endpoint-only table/column lineage, Reusable Mapping instance scoping, SDK read-only safeguards, and deferred export/idempotency decisions | Building or reviewing the standalone ODI Lineage Explorer, validating the real SDK adapter, or resuming future OpenMetadata export design | 2026-09-03 | CURRENT — real 14c table/column lineage and reusable traversal verified; user visual comparison plus no-DML credentials and DB audit evidence remain acceptance gates |
